<template>
  ...
  <v-layout v-else row wrap class="white" color="primary" ma-3>
    ...
    <v-flex xs10 offset-xs1 sm8 offset-sm2 md6 offset-md3 lg4 offset-lg4 xl2 offset-xl5 class="parent text-xs-center">
      <img v-if="showLogo" class="logo" :src="require(`@/assets/${logo}`)">
      <h1 class="app-title primary--text">'global.login.title'</h1>
      <template>
        <v-form v-model="valid" ref="form" lazy-validation v-on:submit.prevent>
          ...
          <v-text-field :label="'global.login.email'" v-model="email" :rules="emailRules" required></v-text-field>
          <v-text-field :label="'global.login.password'" v-model="password" :rules="passwordRules" :counter="30" required :append-icon="passAppendIcon"
...
export default {
  ...
  computed: {
    ...
    passwordRules () {
      return [
        'global.login.passwordRequired'
      ]
    },
    emailRules () {
      return [
        'global.login.emailRequired',
        'global.login.incorrectEmail'
      ]
    },
    credential () {
      const { email } = this
